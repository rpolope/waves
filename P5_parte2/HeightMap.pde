    ...
    for(int i = 0; i < _numNodesX; i++)
      for(int j = 0; j < _numNodesY; j++){
        ...
        for (Wave wave: waves){
          ...
          pos = new PVector(_nodes[i][j].x, _nodes[i][j].y, _nodes[i][j].z);
          newPos = wave.getVariation(_nodes[i][j].x, _nodes[i][j].y, _nodes[i][j].z, _simulatedTime); 
          
            
            if(wave.D.dist(pos) <= 75){
